...
// 導入面板頁面
$dashboard_page_fileName = $jsonData2["pages"]["dashboard"];
$dashboard_page = BASE_DIR . '/application/tepl/' . $teplName . '/' . $dashboard_page_fileName = $jsonData2["pages"]["dashboard"];
// echo $login_page;
if (!is_file($dashboard_page)) {
	error_Logger("此模板/主題登入檔案不存在，請聯絡管理員進行使用模板更改【dashboard: {$dashboard_page_fileName}】",
	 "路徑: {$dashboard_page}");
}else{
	require BASE_DIR . '/application/tepl/' . $teplName . '/' . $dashboard_page_fileName = $jsonData2["pages"]["dashboard"];
}
?>